...
import loadContext, { DEFAULT_CONFIG as config } from './unit/context'
...
testContext('allocate', function () {
  ...
  beforeEach(async function () {
    ...
    await beforeEachAllocate(this.signers, this.contracts)
  })
  ...
  describe('when allocating from margin', function () {
    ...
    describe('fail cases', function () {
      ...
      it('reverts if risky or stable margins are insufficient', async function () {
        await expect(
          this.contracts.router.allocateFromMargin(poolId, this.contracts.router.address, parseWei('10000000').raw, HashZero)
        ).to.be.reverted
      })
  ...
  describe('when allocating from external', function () {
    describe('success cases', function () {
      ...
      it('transfers the tokens', async function () {
        ...
        await this.contracts.router.allocateFromExternal(poolId, this.contracts.router.address, parseWei('1').raw, HashZero)
