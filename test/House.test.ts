...
import { IERC20, PrimitiveHouse, TestCallee, PrimitiveEngine, TestBlackScholes, PrimitiveFactory } from '../typechain'

const { createFixtureLoader } = waffle

describe('Primitive House', function () {
  ...
  const protocolFixture: Fixture<{
    ...
    const { engine, callee, house, TX1, TY2, bs } = await loadFixture(primitiveProtocolFixture)

    ;[TX1, TY2].map(async (token) => {
      await token.approve(house.address, constants.MaxUint256)
      await token.connect(signer2).approve(house.address, constants.MaxUint256)
      await token.transfer(signer2.address, BigNumber.from(1_000_000).mul(BigNumber.from(10).pow(18)))
    })
  ...
  describe('Initialize Primitive and Uniswap v3 contracts', () => {})
