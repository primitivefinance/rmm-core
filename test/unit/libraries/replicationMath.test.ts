...
function step3(reserve: Wei) {
  let inside = 1 - reserve.float
  let expected = inverse_std_n_cdf(inside)
  return expected
}

function step4(reserve: Wei, sigma: number, tau: Time) {
  let vol = getProportionalVol(sigma, tau.years)
  let expected = step3(reserve) - vol
  return expected
}

function step5(reserve: Wei, strike: Wei, sigma: number, tau: Time) {
  const invariant = 0
  let input = step4(reserve, sigma, tau)
...
const rangeTesting: RangeTest = {
  ...
  ['testStep4']: {
    ...
    error: maxError.centralInverseCDF,
    parse: parseWei,
    expected: (val: Wei) => step4(val, 0.1, new Time(Time.YearInSeconds)),
  },
  ['testStep5']: {
    // input
    ...
    error: maxError.cdf,
    parse: parseWei,
    expected: (val: Wei) => step5(val, parseWei(10), 0.1, new Time(Time.YearInSeconds)),
  },
  ['getStableGivenRisky']: {
    // reserveRisky
...
TestPools.forEach(function (pool: PoolState) {
  testContext(`testReplicationMath for ${pool.description}`, function () {
    ...
    describe('testGetStableGivenRisky', function () {
      ...
      for (let step in rangeTesting) {
        describe(`testing ${step}`, function () {
          let range = rangeTesting[step]
