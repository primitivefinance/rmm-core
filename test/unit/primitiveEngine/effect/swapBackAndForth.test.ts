...
testContext('Swap stable to risky back and forth', function () {
  ...
  describe('swaps stable in, then swaps the amount out of risky back in', function () {
    it('swaps back and forth', async function () {
      ...
      expect(riskyDiff).to.be.greaterThanOrEqual(0)
      expect(stableDiff).to.be.greaterThanOrEqual(0)
    })
  })
})
