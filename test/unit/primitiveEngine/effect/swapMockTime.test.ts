...
;[year - 1, year / 1.5, year / 2, year / 3, year / 4].forEach((timeDelta) =>
  testContext(`Swapping after ${new Time(timeDelta).years} years have passed`, function () {
    ...
    beforeEach(async function () {
      ...
      const maturity = Math.floor(swap0.t * Time.YearInSeconds * 1e6) / 1e6
      ...
      // push forward time
      await engine.advanceTime(Math.floor(timeDelta))
    })
    ...
    describe('swap in stable', function () {
      ;[2, 4, 6, 8].forEach((amountIn) =>
        it(`swaps in ${swap0.out0 * amountIn} stable if possible`, async function () {
          const [resRisky, resStable] = [scaleUp(swap0.x0, cal0.decimalsRisky), scaleUp(swap0.y0, cal0.decimalsStable)]
          const pool = new Pool(
            resRisky,
            parseWei('1'),
          ...
          const deltaIn = scaleUp(swap0.out0 * amountIn, cal0.decimalsStable)
